...
let sliderImages = document.querySelector(".slider__img");
    ...
    let infoSiti = document.querySelector(".info-city");
    ...
    function initMenu() {
        image.forEach((image, index) => {
            let menuItem = `<div class="menu__item  n${index} ${index === 0? "active" : ""}" data-index="${index}"><h3>${image.title}</h3></div>`;
    ...
    function initSiti() {
        image.forEach((image, index) => {
            let siti = `<div class="siti-text n${index} ${index === 0? "active" : ""}" data-index="${index}"><h4>${image.citi}</h4></div>`;
    ...
    function initArea() {
        image.forEach((image, index) => {
            let area = `<div class="area-text n${index} ${index === 0? "active" : ""}" data-index="${index}"><h4>${image.area}</h4></div>`;
    ...
    function initTime() {
        image.forEach((image, index) => {
            let time = `<div class="time-text n${index} ${index === 0? "active" : ""}" data-index="${index}"><h4>${image.time}</h4></div>`;
